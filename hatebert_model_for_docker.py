...
def train_epoch_hatebert(model, num_epochs, train_dataloader, optimizer, lr_scheduler, criterion, metrics, device, num_training_steps):
    ...
    for batch in train_dataloader:
        ...
        with torch.no_grad():
            for k in epoch_metrics.keys():
                epoch_metrics[k] += metrics[k](preds.cpu(), model_inputs["labels"].cpu())
...
def evaluate_hatebert_with_bias(model, criterion, metrics, dataloader, device):
    model.eval()
    total_loss = 0
    all_predictions = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for k in epoch_metrics.keys():
                epoch_metrics[k] += metrics[k](predictions.cpu(), model_inputs["labels"].cpu())
    ...
    return overall_accuracy, epoch_loss, epoch_metrics
